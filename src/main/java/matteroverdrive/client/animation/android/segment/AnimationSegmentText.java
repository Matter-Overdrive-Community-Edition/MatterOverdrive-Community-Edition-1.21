...
public class AnimationSegmentText extends AnimationSegment {
  ...
  public String getText(int time) {
    if (animationType == 1) {
      return typingAnimation(ChatFormatting.stripFormatting(string).replaceAll("\\$gre", ChatFormatting.GREEN.toString()).replaceAll("\\$res", ChatFormatting.RESET.toString()).replaceAll("\\$forge", NeoForgeVersion.getVersion()), (time - this.getBegin()), this.getLength());
    }
    return "";
  }
